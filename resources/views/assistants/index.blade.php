...
@section('content')
<div x-data="{ openModal: false, assistant: { id: null, name: '', phone: '', address: '', status: 'active', notes: '' } }" class="space-y-8 pb-8">

    <div class="pt-2">
        <div class="flex flex-col gap-8 md:flex-row justify-between items-start">
            <div>
                <h1 class="text-4xl font-bold text-gray-900">Assistants</h1>
            </div>
            <button @click="assistant = { id: null, name: '', phone: '', address: '', status: 'active', notes: '' }; openModal=true"
                ...
                <div class="bg-gradient-to-r from-indigo-500 via-indigo-500 to-indigo-600 px-6 py-5 relative">
                    ...
                    <div class="relative">
                        <div class="w-12 h-12 rounded-lg bg-white/20 flex items-center justify-center mb-3 backdrop-blur-sm">
                            <span class="text-white text-lg font-bold">{{ strtoupper(substr($a->name, 0, 2)) }}</span>
                        </div>
                        <h3 class="text-white font-bold text-lg line-clamp-2">{{ $a->name }}</h3>
                    </div>
                </div>

                <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex gap-2">
                    <a href="{{ route('assistants.show', $a->id) }}"
                    ...
                    <button @click="assistant = item; openModal=true"
                    ...
                    <form action="{{ route('assistants.destroy', $a->id) }}" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                ...
                                onclick="return confirm('Delete this assistant? This action cannot be undone.')">
    ...
    @else
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div class="text-center py-20 px-6">
                ...
                <h3 class="text-2xl font-bold text-gray-900 mb-2">No Assistants Yet</h3>
                <p class="text-gray-600 mb-8 max-w-sm mx-auto">Add assistants to help with deliveries and assignments.</p>
                <button @click="assistant = { id: null, name: '', phone: '', address: '', status: 'active', notes: '' }; openModal=true"
    ...
    <div x-show="openModal" @keydown.escape="openModal=false" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" style="display: none;">
        <div @click.away="openModal=false" class="bg-white rounded-xl w-full max-w-2xl p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="mb-6 sticky top-0 bg-white">
                <h2 class="text-3xl font-bold text-gray-900" x-text="assistant.id ? 'Edit Assistant' : 'Add New Assistant'"></h2>
                <p class="text-gray-600 text-sm mt-2" x-text="assistant.id ? 'Update assistant information' : 'Create a new assistant'"></p>
            </div>

            <form :action="assistant.id ? '/assistants/' + assistant.id : '/assistants'" method="POST">
                @csrf
                <template x-if="assistant.id"><input type="hidden" name="_method" value="PUT"></template>

                <x-form.error-summary />

                <div class="space-y-5 mb-6">
                    <x-form.input name="name" label="Name" placeholder="Full name" required x-model="assistant.name" />
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-form.input name="phone" label="Phone" placeholder="Phone number" x-model="assistant.phone" />
                        <x-form.input name="address" label="Address" placeholder="Address (optional)" x-model="assistant.address" />
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Status</label>
                            <select name="status" x-model="assistant.status" class="mt-1 w-full rounded border-gray-300">
                        ...
                        <div>
                            ...
                            <input name="notes" x-model="assistant.notes" class="mt-1 w-full rounded border-gray-300" placeholder="Optional notes" />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-gray-200 sticky bottom-0 bg-white">
                    ...
                    <x-form.button type="submit" variant="primary" size="md">Save Assistant</x-form.button>
                </div>
            </form>
        </div>
    </div>
